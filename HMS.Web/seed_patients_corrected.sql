-- Corrected Patient Seeding Script
USE [HospitalManagement];
GO

-- Seed 1000 Patients with all required fields
DECLARE @i INT = 1;
WHILE @i <= 1000
BEGIN
    INSERT INTO Patients (
        UserId, FullName, DateOfBirth, Gender, ContactNumber, Email, Address,
        City, State, Country, ZipCode, BloodGroup, EmergencyContactName, 
        EmergencyContactNumber, RelationToEmergencyContact, Allergies, 
        ChronicDiseases, CurrentMedications, DisabilityStatus, RegistrationDate,
        IsActive, PatientType, PrimaryDoctorId, LastVisitDate
    )
    VALUES (
        NULL,
        ...
        CAST(@i AS NVARCHAR) + ' Hospital Street',
        'Medical City',
        'State ' + CAST((@i % 50) AS NVARCHAR),
        'USA',
        CAST((10000 + @i) AS NVARCHAR),
